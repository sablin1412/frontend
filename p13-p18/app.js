...
async function subscribeToPush() {
    ...
    try {
        ...
        const subscription = await registration.pushManager.subscribe({
            ...
            applicationServerKey: urlBase64ToUint8Array('BHBF5lL8dAg1xJFHIl2-DlSJ6FnSUhFBZiGpYMO9J9TJ04xT5oJpYTzG9xZECq84txm8DAUgMFi-1qRhuxDSJko')
        });
    ...
    } catch (err) { console.error('Ошибка подписки на push:', err); }
}
    ...
    window.loadNotes = function() {
        ...
        list.innerHTML = notes.map((note, index) => {
            ...
            if (note.reminder) {
                ...
                reminderInfo = `<br><small class="text-error">⏰ Напоминание: ${date.toLocaleString()}</small>`;
            }
            return `
            <li class="card" style="margin-bottom: 0.5rem; padding: 1rem; display: flex; justify-content: space-between;">
                ...
                <button class="button primary" onclick="deleteNote(${index})">Удалить</button>
            </li>`;
        }).join('');
    };
